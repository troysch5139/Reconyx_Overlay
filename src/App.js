...
function App() {
  ...
  const drawCanvas = () => {
    ...
    // Set canvas size to accommodate both images (whichever is larger)
    canvas.width = Math.max(baseImage.naturalWidth, overlayImage.naturalWidth);
    canvas.height = Math.max(baseImage.naturalHeight, overlayImage.naturalHeight);

    // Draw base image at original size
    ctx.drawImage(baseImage, 0, 0, baseImage.naturalWidth, baseImage.naturalHeight);

    // Draw overlay at its original size (no stretching)
